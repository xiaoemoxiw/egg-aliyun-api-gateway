...
class Client extends Base {
  ...
  async request(method, url, opts, originData) {
    ...
    const response = await httpx.request(parsedUrl, {
      method,
      headers,
      data: opts.data,
      beforeRequest(opts) {
    ...
    if (contentType.startsWith('application/json') && result) {
      result = JSON.parse(result);
    } else if (Reflect.has(response.headers, 'x-ca-error-message')
            && response.headers['x-ca-error-message'] !== 'OK') {
      result = {
